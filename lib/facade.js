 ...
 *
 * @param {Object} obj - The object to convert into objects.
 ...
 * Identify} overrides this implementation.
 *
 * Traits are gotten from `options.traits`, augmented with a property `id` with
 * the event `userId`.
 *
 * For each `aliasFrom` key in `aliases` with value `aliasTo`,
 * `options.traits[aliasFrom]` will be removed and reinserted at the key
 * `aliasTo`. If `aliasFrom` is a property of this facade (as opposed to a
 * property of the wrapped object), then it's assumed that `aliasFrom` is the
 * name of a function (e.g. `firstName`), and will be invoked and the result
 * placed under `aliasTo`, instead of whatever was in
 * `options.traits[aliasFrom]`.
 *
