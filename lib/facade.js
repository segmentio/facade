...
Facade.prototype.proxy = function (field) {
  ...
  const obj = this[field] || this.field(field);
  if (!obj) return obj;
...
Facade.multi = function (path) {
  return function () {
    const multi = this.proxy(path + "s");
    if (type(multi) === "array") return multi;
    const one = this.proxy(path);
...
Facade.prototype.json = function () {
  const ret = this.opts.clone ? clone(this.obj) : this.obj;
...
Facade.prototype.options = function (integration) {
  ...
  const value = integrations[integration] || objCase(integrations, integration);
...
Facade.prototype.enabled = function (integration) {
  const allEnabled = this.proxy("options.providers.all");
  if (typeof allEnabled !== "boolean") allEnabled = this.proxy("options.all");
  if (typeof allEnabled !== "boolean")
    allEnabled = this.proxy("integrations.all");
  if (typeof allEnabled !== "boolean") allEnabled = true;

  const enabled = allEnabled && isEnabled(integration);
...
Facade.prototype.active = function () {
  const active = this.proxy("options.active");
 ...
 *
 * @example
 * const obj = { options: { traits: { foo: "bar" } }, anonymousId: "xxx" }
 * const facade = new Facade(obj)
 *
...
Facade.prototype.traits = function (aliases) {
  const ret = this.proxy("options.traits") || {};
...
Facade.prototype.library = function () {
  const library = this.proxy("options.library");
...
Facade.prototype.device = function () {
  const device = this.proxy("context.device");
